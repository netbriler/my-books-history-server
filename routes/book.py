...
from services.books import search_books, get_book_from_google, get_or_create_book, get_all_by_user_id
...
async def search(q: str = Query(...), start_index: int = Query(0, alias='startIndex', ge=0),
    ...
    external_books = await get_all_by_user_id(current_user.id)
